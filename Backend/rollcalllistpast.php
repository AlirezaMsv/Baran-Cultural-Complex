<?php
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if ($flag==0){
			$sql = "SELECT `firstname`,`lastname`,`kodemelli`,`flag` from clienttbl WHERE (`flag`='0' OR `flag`='1') AND(`appsentdates` LIKE '%$date%' OR `presendates` LIKE '%$date%')";}
		else{
			if ($category==0){
				$sql = "SELECT `firstname`,`lastname`,`kodemelli`,`flag` from clienttbl WHERE (`flag`='0' OR `flag`='1') AND (`appsentdates` LIKE '%$date%' OR `presendates` LIKE '%$date%') AND `firstname` LIKE  '%$firstname%'
